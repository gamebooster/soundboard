...
static void xm_arpeggio(xm_context_t*, xm_channel_context_t*, uint8_t,
                        uint16_t);
...
static float xm_envelope_lerp(xm_envelope_point_t*, xm_envelope_point_t*,
                              uint16_t);
static void xm_envelope_tick(xm_channel_context_t*, xm_envelope_t*, uint16_t*,
                             float*);
...
static void xm_handle_note_and_instrument(xm_context_t*, xm_channel_context_t*,
                                          xm_pattern_slot_t*);
...
#define XM_SLIDE_TOWARDS(val, goal, incr) \
  do {                                    \
    if ((val) > (goal)) {                 \
      (val) -= (incr);                    \
...
#define HAS_TONE_PORTAMENTO(s)                       \
  ((s)->effect_type == 3 || (s)->effect_type == 5 || \
   ((s)->volume_column >> 4) == 0xF)
#define HAS_ARPEGGIO(s) ((s)->effect_type == 0 && (s)->effect_param != 0)
#define HAS_VIBRATO(s)                                \
  ((s)->effect_type == 4 || (s)->effect_param == 6 || \
...
static void xm_autovibrato(xm_context_t* ctx, xm_channel_context_t* ch) {
  ...
  if (ch->autovibrato_ticks < instr->vibrato_sweep) {
    ...
    sweep = XM_LERP(0.f, 1.f,
                    (float)ch->autovibrato_ticks / (float)instr->vibrato_sweep);
  }
...
static void xm_tremolo(xm_context_t* ctx, xm_channel_context_t* ch,
                       uint8_t param, uint16_t pos) {
  (void)(ctx);
  unsigned int step = pos * (param >> 4);
...
static void xm_tone_portamento(xm_context_t* ctx, xm_channel_context_t* ch) {
  ...
  if (ch->period != ch->tone_portamento_target_period) {
    XM_SLIDE_TOWARDS(
        ...
        (ctx->module.frequency_type == XM_LINEAR_FREQUENCIES ? 4.f : 1.f) *
...
static float xm_envelope_lerp(xm_envelope_point_t* restrict a,
                              xm_envelope_point_t* restrict b, uint16_t pos) {
...
static float xm_linear_period(float note) { return 7680.f - note * 64.f; }
  ...
  switch (ctx->module.frequency_type) {
    ...
    case XM_AMIGA_FREQUENCIES:
      ...
      if (XM_DEBUG && (p1 < period || p2 > period)) {
        DEBUG("%i <= %f <= %i should hold but doesn't, this is a bug", p2,
              period, p1);
      }
...
static void xm_handle_note_and_instrument(xm_context_t* ctx,
  ...
  if (s->instrument > 0) {
    if (HAS_TONE_PORTAMENTO(ch->current) && ch->instrument != NULL &&
      ...
      xm_trigger_note(ctx, ch,
                      XM_TRIGGER_KEEP_PERIOD | XM_TRIGGER_KEEP_SAMPLE_POSITION);
  ...
  if (NOTE_IS_VALID(s->note)) {
    ...
    if (HAS_TONE_PORTAMENTO(ch->current) && instr != NULL &&
        ch->sample != NULL) {
  ...
  switch (s->volume_column >> 4) {
    ...
    case 0xA: /* Set vibrato speed */
      ch->vibrato_param =
          (ch->vibrato_param & 0x0F) | ((s->volume_column & 0x0F) << 4);
      break;

    case 0xC: /* Set panning */
      ch->panning = (float)(((s->volume_column & 0x0F) << 4) |
  ...
  switch (s->effect_type) {
    ...
    case 4: /* 4xy: Vibrato */
      if (s->effect_param & 0x0F) {
        /* Set vibrato depth */
        ch->vibrato_param =
            (ch->vibrato_param & 0xF0) | (s->effect_param & 0x0F);
      }
      if (s->effect_param >> 4) {
        /* Set vibrato speed */
        ch->vibrato_param =
    ...
    case 7: /* 7xy: Tremolo */
      if (s->effect_param & 0x0F) {
        /* Set tremolo depth */
        ch->tremolo_param =
            (ch->tremolo_param & 0xF0) | (s->effect_param & 0x0F);
      }
      if (s->effect_param >> 4) {
        /* Set tremolo speed */
        ch->tremolo_param =
    ...
    case 9: /* 9xx: Sample offset */
      if (ch->sample != NULL && NOTE_IS_VALID(s->note)) {
        uint32_t final_offset = s->effect_param
                                << (ch->sample->bits == 16 ? 7 : 8);
    ...
    case 0xC: /* Cxx: Set volume */
      ch->volume = (float)((s->effect_param > 0x40) ? 0x40 : s->effect_param) /
                   (float)0x40;
      break;
        ...
        case 5: /* E5y: Set finetune */
          if (NOTE_IS_VALID(ch->current->note) && ch->sample != NULL) {
            ch->note = ch->current->note + ch->sample->relative_note +
                       (float)(((s->effect_param & 0x0F) - 8) << 4) / 128.f -
                       1.f;
    ...
    case 16: /* Gxx: Set global volume */
      ctx->global_volume =
          (float)((s->effect_param > 0x40) ? 0x40 : s->effect_param) /
          (float)0x40;
      break;
      ...
      if (s->effect_param > 0) {
        if ((s->effect_param >> 4) == 0) {
          ...
          ch->multi_retrig_param =
              (ch->multi_retrig_param & 0xF0) | (s->effect_param & 0x0F);
        } else {
...
static void xm_row(xm_context_t* ctx) {
  ...
  xm_pattern_t* cur = ctx->module.patterns +
  ...
  for (uint8_t i = 0; i < ctx->module.num_channels; ++i) {
    xm_pattern_slot_t* s =
        cur->slots + ctx->current_row * ctx->module.num_channels + i;
  ...
  if (!in_a_loop) {
    ...
    ctx->loop_count =
        (ctx->row_loop_count[MAX_NUM_ROWS * ctx->current_table_index +
                             ctx->current_row]++);
  }
...
static void xm_envelope_tick(xm_channel_context_t* ch, xm_envelope_t* env,
                             uint16_t* counter, float* outval) {
  if (env->num_points < 2) {
    ...
    *outval = xm_envelope_lerp(env->points + j, env->points + j + 1, *counter) /
              (float)0x40;
...
static void xm_envelopes(xm_channel_context_t* ch) {
  if (ch->instrument != NULL) {
    if (ch->instrument->volume_envelope.enabled) {
      ...
      xm_envelope_tick(ch, &(ch->instrument->volume_envelope),
                       &(ch->volume_envelope_frame_count),
                       &(ch->volume_envelope_volume));
    }

    if (ch->instrument->panning_envelope.enabled) {
      xm_envelope_tick(ch, &(ch->instrument->panning_envelope),
...
static void xm_tick(xm_context_t* ctx) {
  ...
  for (uint8_t i = 0; i < ctx->module.num_channels; ++i) {
    ...
    switch (ch->current->effect_type) {
      ...
      case 27: /* Rxy: Multi retrig note */
        ...
        if ((ctx->current_tick % (ch->multi_retrig_param & 0x0F)) == 0) {
          float v =
              ch->volume * multi_retrig_multiply[ch->multi_retrig_param >> 4] +
      ...
      case 29: /* Txy: Tremor */
        ...
        ch->tremor_on =
            ((ctx->current_tick - 1) %
                 ((ch->tremor_param >> 4) + (ch->tremor_param & 0x0F) + 2) >
             (ch->tremor_param >> 4));
        break;
    ...
    panning = ch->panning + (ch->panning_envelope_panning - .5f) *
                                (.5f - fabsf(ch->panning - .5f)) * 2.0f;
...
static float xm_sample_at(xm_sample_t* sample, size_t k) {
  return sample->bits == 8 ? (sample->data8[k] / 128.f)
                           : (sample->data16[k] / 32768.f);
}
...
static float xm_next_of_sample(xm_channel_context_t* ch) {
  ...
  switch (ch->sample->loop_type) {
    ...
    case XM_FORWARD_LOOP:
      if (XM_LINEAR_INTERPOLATION) {
        v = xm_sample_at(ch->sample, (b == ch->sample->loop_end)
                                         ? ch->sample->loop_start
                                         : b);
      }
      ch->sample_position += ch->step;
    ...
    case XM_PING_PONG_LOOP:
      ...
      if (ch->ping) {
        ...
        if (ch->sample_position >= ch->sample->loop_end) {
          ch->ping = false;
          ch->sample_position =
              (ch->sample->loop_end << 1) - ch->sample_position;
        }
        /* sanity checking */
        if (ch->sample_position >= ch->sample->length) {
          ch->ping = false;
        ...
        if (ch->sample_position <= ch->sample->loop_start) {
          ch->ping = true;
          ch->sample_position =
              (ch->sample->loop_start << 1) - ch->sample_position;
        }
        /* sanity checking */
  ...
  *right *= fgvol;
  ...
  if (XM_DEBUG) {
    if (fabs(*left) > 1 || fabs(*right) > 1) {
      DEBUG("clipping frame: %f %f, this is a bad module or a libxm bug", *left,
            *right);
    }
  }
}
