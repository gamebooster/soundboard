...
int xm_create_context_safe(xm_context_t** ctxp, const char* moddata,
                           size_t moddata_length, uint32_t rate) {
  size_t bytes_needed;
  char* mempool;
  xm_context_t* ctx;
  ...
  if (XM_DEFENSIVE) {
    ...
    if ((ret = xm_check_sanity_preload(moddata, moddata_length))) {
      DEBUG("xm_check_sanity_preload() returned %i, module is not safe to load",
            ret);
      return 1;
    }
  }
  ...
  ctx->amplification = .25f; /* XXX: some bad modules may still clip. Find out
                                something better. */
  ...
  if (XM_DEFENSIVE) {
    ...
    if ((ret = xm_check_sanity_postload(ctx))) {
      DEBUG(
          "xm_check_sanity_postload() returned %i, module is not safe to play",
          ret);
      xm_free_context(ctx);
      return 1;
    }
...
void xm_create_context_from_libxmize(xm_context_t** ctxp, char* libxmized,
                                     uint32_t rate) {
  size_t i, j;
  (void)(rate);
      ...
      if (XM_LIBXMIZE_DELTA_SAMPLES) {
        if ((*ctxp)->module.instruments[i].samples[j].length > 1) {
          if ((*ctxp)->module.instruments[i].samples[j].bits == 8) {
            for (size_t k = 1;
                 k < (*ctxp)->module.instruments[i].samples[j].length; ++k) {
              (*ctxp)->module.instruments[i].samples[j].data8[k] +=
                  (*ctxp)->module.instruments[i].samples[j].data8[k - 1];
            }
          } else {
            for (size_t k = 1;
                 k < (*ctxp)->module.instruments[i].samples[j].length; ++k) {
              (*ctxp)->module.instruments[i].samples[j].data16[k] +=
                  (*ctxp)->module.instruments[i].samples[j].data16[k - 1];
            }
          }
        }
      }
...
void xm_free_context(xm_context_t* context) { free(context); }

void xm_set_max_loop_count(xm_context_t* context, uint8_t loopcnt) {
  context->max_loop_count = loopcnt;
}

uint8_t xm_get_loop_count(xm_context_t* context) { return context->loop_count; }
...
#if XM_STRINGS
const char* xm_get_module_name(xm_context_t* ctx) { return ctx->module.name; }
...
uint16_t xm_get_module_length(xm_context_t* ctx) { return ctx->module.length; }
...
void* xm_get_sample_waveform(xm_context_t* ctx, uint16_t i, uint16_t s,
                             size_t* size, uint8_t* bits) {
...
void xm_get_position(xm_context_t* ctx, uint8_t* pattern_index,
...
uint64_t xm_get_latest_trigger_of_instrument(xm_context_t* ctx,
                                             uint16_t instr) {
  return ctx->module.instruments[instr - 1].latest_trigger;
}

uint64_t xm_get_latest_trigger_of_sample(xm_context_t* ctx, uint16_t instr,
                                         uint16_t sample) {
...
bool xm_is_channel_active(xm_context_t* ctx, uint16_t chn) {
  ...
  return ch->instrument != NULL && ch->sample != NULL &&
